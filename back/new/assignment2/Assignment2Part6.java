...
public class Assignment2Part6 extends WindowProgram {
    ...
    /**
     * Build caterpillar where each subsequent segment is shifted up or down(regulates by boolean variable buildUp).
     * For every segment calculates posX and posY(calculates by function getYPos(...)) then builds segment by function
     * buildOneSegment(...).
     */
    ...
    /**
     * Function which calculates Y-coordinate of left up corner of one segment of caterpillar.
     *
     * @param posY double value of Y-coordinate of previous segment
     * @param buildUp boolean value which manages where current segment is built(top or bottom row)
     *
     * @return double value of Y-coordinate of current segment
     * */
    private double getYPos(double posY, boolean buildUp) {
        if (buildUp) {
            posY -= OVAL_DIAMETER / 2;
        } else {
            posY += OVAL_DIAMETER / 2;
        }
        return posY;
