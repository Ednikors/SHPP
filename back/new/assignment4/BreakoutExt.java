...
public class BreakoutExt extends WindowProgram {
     ...
     */
    private static final double PAUSE_TIME = 1000.0 / 150;
    ...
    private final GLabel[] heartBar = new GLabel[3];
    ...
    private void gameProcess(int roundNum, GObject ball, GRect dataBar) {
        ...
        while (brickAmount > 0) {
            ...
            if (ball.getY() + BALL_RADIUS * 2 >= getHeight()) {
                ...
                heartBar[(2 - roundNum)].setColor(Color.GRAY);
     ...
     */
    private GLabel drawHeart(int heartNum) {
        String symbol = "\u2665"; // Change this to the symbol you want to use

        GLabel label = new GLabel(symbol);
        label.setColor(Color.RED);
        label.setFont("SansSerif-Bold-24"); // You can adjust the font and size

        double x = BRICK_SEP + heartNum * (label.getWidth() + BRICK_SEP);
        double y = BRICK_SEP + HEART_BAR_HEIGHT / 2 + label.getAscent() / 2;
